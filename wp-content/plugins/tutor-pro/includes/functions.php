 ...
 * @return object
 */
function get_generated_bus( $config = array() ) {
	global $wpdb;
	...
	if ( $course_id ) {
		$filter_sql .= " AND bus_result.course_id = {$course_id} ";
	}
	if ( $term ) {
		$filter_sql .= " AND (course.post_title LIKE '%{$term}%' OR student.display_name LIKE '%{$term}%' ) ";
	}
	if ( '' !== $date ) {
		$filter_sql .= " AND DATE(bus_result.update_date) = CAST('$date' AS DATE) ";
	}

	$bus['count'] = $wpdb->get_var(
		"SELECT COUNT(bus_result.bus_result_id) total_res
				FROM {$wpdb->tutor_buss_results} bus_result
					LEFT JOIN {$wpdb->posts} course ON bus_result.course_id = course.ID
					LEFT  JOIN {$wpdb->users} student ON bus_result.user_id = student.ID
				WHERE bus_result.result_for = 'final' {$filter_sql} ;
			"
	);
	$bus['res']   = $wpdb->get_results(
		"SELECT bus_result.*, 
			(SELECT COUNT(quizzes.quiz_id) FROM {$wpdb->tutor_buss_results} quizzes WHERE quizzes.user_id = bus_result.user_id AND quizzes.course_id = bus_result.course_id AND quizzes.result_for = 'quiz') as quiz_count,

			(SELECT COUNT(assignments.assignment_id) FROM {$wpdb->tutor_buss_results} assignments WHERE assignments.user_id = bus_result.user_id AND assignments.course_id = bus_result.course_id AND assignments.result_for = 'assignment') as assignment_count,
		grade_config,
		student.display_name,
		course.post_title as course_title
		FROM {$wpdb->tutor_buss_results} bus_result
			LEFT JOIN {$wpdb->tutor_buss} bus ON bus_result.bus_id = bus.bus_id
			LEFT JOIN {$wpdb->posts} course ON bus_result.course_id = course.ID
			LEFT  JOIN {$wpdb->users} student ON bus_result.user_id = student.ID
		WHERE bus_result.result_for = 'final' {$filter_sql} 
		ORDER BY bus_result.generate_date {$order}
		LIMIT {$start}, {$limit}
		"
	);
